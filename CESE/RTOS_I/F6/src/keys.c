...
extern QueueSetHandle_t Cola1, Cola2;
...
static void buttonReleased( uint32_t index )
{
    ...
    if ( keys_data[index].time_diff  > 0 )
    {
        ...
        Parameters_t Parameters = { 1 , index , keys_data[index].time_diff};
        
         //xSemaphoreGive(mutex);
        
        xQueueSend(Cola1,&Parameters, portMAX_DELAY);
         
        xQueueSend(Cola2,&Parameters, portMAX_DELAY);
    }
